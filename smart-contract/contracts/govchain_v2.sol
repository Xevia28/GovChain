...
contract GovChain_v2 is Ownable {
    ...
    GovChainToken_v2 public token; // to refer our token (GovChainToken_v2)

    // IERC20 public token; // to refer our token (GovChainToken_v2)

    // we set our token variable to the deployed token (GovChainToken_v2) contract address in our constructor
    // constructor(address _tokenAddress) Ownable() {
    constructor() Ownable() {
        // token = IERC20(_tokenAddress);
        token = new GovChainToken_v2();
    }
    ...
    function _closeTopic(uint256 _topicId) private {
        ...
        uint256 totalVotes = topic.yesVotes +
            topic.noVotes +
            topic.neutralVotes;

        uint256 majorityThreshold = (totalVotes / 2) + 1; // Majority threshold is more than half of the votes cast

        uint256 twoThirdsThreshold = (totalVotes * 2) / 3; // Two-thirds threshold is 2/3 of the total votes
